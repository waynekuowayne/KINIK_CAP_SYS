...
def update_machine_status(status):
    ...
    try:
        ...
        write_log(f"✗ 機台狀態更新錯誤: {e}")
        raise  # 重新拋出錯誤，讓外層 OperationalError 處理重連
...
def sync_task_to_database():
    ...
    try:
        ...
        # 如果沒有任務名稱
        if not task_name:
            # 但如果有進行中的任務 ID，表示 VB 程式已關閉或清空 Registry
            if current_task_db_id > 0:
                write_log(f"⚠ 任務已被清空，檢查任務狀態，Task ID: {current_task_db_id}")
                cursor = db_connection.cursor()
                # 查詢任務的進度和總數
                cursor.execute("SELECT progress, total_images FROM tasks WHERE id = %s", (current_task_db_id,))
                result = cursor.fetchone()
                if result:
                    current_progress = result[0]
                    final_total = result[1]

                    # 判斷任務是否真的完成
                    if current_progress >= final_total:
                        # 進度已達到或超過總數 → 正常完成
                        sql = """
                            UPDATE tasks
                            SET progress = %s, status = 'COMPLETED', actual_completion_time = NOW(), updated_at = NOW()
                            WHERE id = %s
                        """
                        cursor.execute(sql, (final_total, current_task_db_id))
                        db_connection.commit()
                        write_log(f"✓ 任務正常完成，標記為 COMPLETED: {final_total}/{final_total}")
                    else:
                        # 進度未達到總數 → 異常中斷，標記為失敗
                        sql = """
                            UPDATE tasks
                            SET status = 'FAILED', actual_completion_time = NOW(), updated_at = NOW()
                            WHERE id = %s
                        """
                        cursor.execute(sql, (current_task_db_id,))
                        db_connection.commit()
                        write_log(f"✗ 任務異常中斷，標記為 FAILED: {current_progress}/{final_total}")
                cursor.close()
                current_task_db_id = 0
            return
        ...
        # 更新任務進度
        # 如果狀態是 completed 但進度小於總數，強制設定為總數（避免 VB 太快清空導致遺漏）
        if status == "completed" and progress < total_images:
            write_log(f"⚠ 任務標記為完成但進度不足，自動修正: {progress} → {total_images}")
